  ...
  return (
    <div className="stories-bar">
      ...
      {myGroup && myGroup.stories && myGroup.stories.length > 0 && (() => {
        const firstUnreadStory = myGroup.stories.find((s) => !s.seen);
        const previewImage = firstUnreadStory?.media_url || myGroup.stories[0]?.media_url || "https://images.unsplash.com/photo-1514525253161-7a46d19cd819?w=150";
        return (
          <div
      ...
      {otherGroups.map((group) => {
        // Display the first unread slide's media_url as card preview background, fallback to the first slide
        const firstUnreadStory = group.stories?.find((s) => !s.seen);
        const previewImage = firstUnreadStory?.media_url || group.stories?.[0]?.media_url || "https://images.unsplash.com/photo-1514525253161-7a46d19cd819?w=150";
        return (
          <div
            key={group.user_id}
            className="story-card"
